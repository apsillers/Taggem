module.exports = function(changeListener, mapData, entities, activeEntities, entitiesByLocation, genId, playerKnowledge) {
    var ROT = require("rot");
    ...
    var utilities = {
        ...
        // the player can see inputEntities; we are trying to communicate that set of entities
        // the player has a set of the last entities he could see
        // so we diff these sets
        diffEntitiesForPlayer: function(id, inputEntities) {
            var entitiesDiff = { "add": {}, "remove": [] };
            if(playerKnowledge[id].entities == undefined) {
                playerKnowledge[id].entities = {};
            }
            var playerKnownEntities = playerKnowledge[id].entities;
            for(var i in inputEntities) {
                // if entity is not yet known
                if(playerKnownEntities[i] == undefined) {
                    playerKnownEntities[i] = inputEntities[i];
                    entitiesDiff["add"][i] = inputEntities[i];
                } else {
                    var fields = ['x','y','z','symbol','color','blocking','name'];
                    for(var j=0; j<fields.length; ++j) {
                        // if the entity is known but differs in any field, it has changed and should be replaced
                        if(playerKnownEntities[i][fields[j]] != inputEntities[i][fields[j]]) {
                            entitiesDiff["add"][i] = inputEntities[i];
                            playerKnownEntities[i] = inputEntities[i];
                            break;
                        }
            ...
            // remove entites the player used to see but does not any more
            for(i in playerKnownEntities) {
                if(inputEntities[i] == undefined) {
        ...
        lightPassesOnLevel: function (ents, level) {
            ...
            return function(x, y) {
                ...
                var here = utilities.getEntitiesByLocation(level, x, y, ents);
