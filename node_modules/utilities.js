module.exports = function(listeners, state, genId, playerKnowledge) {
    var ROT = require("rot");
    ...
    var utilities = {
        ...
        diffMapForPlayer: function(id, mapData) {
            var mapDataDiff = {};
            if(playerKnowledge[id].map[state.entities[id].z] == undefined) {
                playerKnowledge[id].map[state.entities[id].z] = {};
            }
            var playerMap = playerKnowledge[id].map[state.entities[id].z];
        ...
        diffEntitiesForPlayer: function(id, visibleEntities) {
            ...
            if(playerKnowledge[id].entities[state.entities[id].z] == undefined) {
                playerKnowledge[id].entities[state.entities[id].z] = {};
            }
            var playerKnownEntities = playerKnowledge[id].entities[state.entities[id].z];
        ...
        passableOnLevel: function (ents, level, start, dest) {
            // determines if light can pass through a given (x,y)
            return function(x, y) {
                if (state.mapData[level] == undefined ||
                    state.mapData[level][x] == undefined || 
                ...
                for (var i = 0; i < here.length; i++) {
                    if (here[i].blocking && (here[i].x != dest.x || here[i].y != dest.y) && (here[i].x != start.x || here[i].y != start.y)) {
                      return false;
                    }
                }
                return true;
